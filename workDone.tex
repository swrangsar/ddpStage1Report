...
The `decimation' is defined as `dsp rate' divided by `sample rate'. The UHD driver requires the decimation value to be an even number. The dsp rate of the USRP is 100MHz. Hence we chose sampling frequency to be 10 MHz which gives a decimation value of: 100 MHz / 10 MHz = 10.
...
We have been able to successfully sense the presence of primary user in the predefined frequency band and as soon as the primary user leaves the energy in that band goes low and we are able to switch on the secondary BTS and secondary users are able to make calls and transmit SMSÕs using secondary BTS. 
